...
const string strMessageMagic = "BitcoinAir Signed Message:\n";
...
static const int64 nTargetSpacing = 2.5 * 60; // BitcoinAir: 2.5 minutes between block
...
void ThreadScriptCheck() {
    RenameThread("BitcoinAir-scriptch");
...
{
    if (nHeight == -1) {
        if (GetBlockTime() == 1546629098)
            nHeight=1;
        else if (GetBlockTime() < 1546944024)
            nHeight=31000;
        else if (GetBlockTime() < 1547364103)
            nHeight=41000;
        else
            nHeight=41001;
    }
...
bool LoadBlockIndex()
{
    ...
    {
#ifdef TESTING
        hashGenesisBlock = uint256("0x000003e8d6924aba5397cfa6d8ababe4f89384b4334cca6f823565c3ccf7799b");
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "BitcoinAir", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ASOC share price falls 40%"; //timestamp
        CTransaction txNew;
        txNew.nTime = 1545089295;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(9999) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].SetEmpty();//scriptPubKey = CScript() << ParseHex("04f546842c0e23a33e826f24e9b01aacf71f3c868858f4fb2f781cba5fd6419727ae707db08fc1eb8b35c14a6598bf7579bd547aa5823f6d02af3f009356683fed") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1545089295;
        block.nBits    = bnProofOfWorkLimit.GetCompact();;
        block.nNonce   = 1668558;

        if (fTestNet)
        {
            block.nTime    = 1545089295;
            block.nNonce   = 168370427;
        }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("BitcoinAirMiner : proof-of-work not meeting target");

    //// debug print
    printf("BitcoinAirMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinAirMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinAirMiner : ProcessBlock, block not accepted");
    }
...
#endif
{
    ...
    try { ploop {
        ...
        printf("Running BitcoinAirMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("BitcoinAirMiner terminated\n");
        throw;
    }
}
