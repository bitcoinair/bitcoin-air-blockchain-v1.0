...
#else
    const char* pszModule = "BitcoinAir";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinAir
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinAir
    // Mac: ~/Library/Application Support/BitcoinAir
    // Unix: ~/.BitcoinAir
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinAir";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "BitcoinAir.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "BitcoinAird.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BitcoinAir will not work properly.");
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "BitcoinAir:" << FormatVersion(BITCOINAIR_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
